...
namespace AquaCulture
{
    public class PlayerInput : InputComponent, IDataPersister
    {
        ...
        public InputButton Inventory = new InputButton(KeyCode.I, XboxControllerButtons.LeftBumper);
        ...
        protected override void GetInputs(bool fixedUpdateHappened)
        {
            ...
            Inventory.Get(fixedUpdateHappened, inputType);
        ...
        void OnGUI()
        {
            if (m_DebugMenuIsOpen)
            {
                ...
                GUILayout.Label("Enter cheat item followed by amount");
                cheatItem = GUILayout.TextField(cheatItem, 25,"textField");
                int.TryParse(GUILayout.TextField(cheatAmount.ToString()), out cheatAmount);

                if (GUILayout.Button("Cheat"))
                {
                    if ((cheatItem == "Jules") || (cheatItem == "cigarMachine") || (cheatItem == "seaweed") || (cheatItem == "cigar")
                        || (cheatItem == "oxygenUpgrade") || (cheatItem == "pressureUpgrade") || (cheatItem == "cargoCapUpgrade"))
                    {
                        Debug.Log("added " + cheatAmount + " " + cheatItem + " to player inventory.");
                        for (int i = 0; i < cheatAmount; i++) { PlayerInventory.AddItem(cheatItem); }
                    }
                    else { Debug.Log("You can't put that in your inventory, you fool!");  }
                }
