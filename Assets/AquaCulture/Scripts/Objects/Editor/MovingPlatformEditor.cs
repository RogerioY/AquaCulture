...
namespace AquaCulture
{
    ...
    public class MovingPlatformEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            float newSpeed = EditorGUILayout.FloatField("Speed", m_MovingPlatform.m_Speed);
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(target, "Changed Speed");
                m_MovingPlatform.m_Speed = newSpeed;
            }
        ...
        private void OnSceneGUI()
        {
            ...
            for (int i = 0; i < m_MovingPlatform.localNodes.Length; ++i)
            {
                ...
                {
                    // YJ: This is messy, create an accessor on the platform class in future
                    if ( m_MovingPlatform.transform.parent != null )
                    {
                        worldPos = m_MovingPlatform.transform.parent.TransformPoint( m_MovingPlatform.worldNode[i] );
                    }
                    else
                    {
                ...
                if (i == 0)
                {
                    ...
                    if (Application.isPlaying)
                    {
                        ...
                        if ( m_MovingPlatform.transform.parent != null )
                        {
                            Handles.DrawDottedLine( worldPos, m_MovingPlatform.transform.parent.TransformPoint( m_MovingPlatform.worldNode[m_MovingPlatform.worldNode.Length - 1] ), 10 );
                        }
                        else
                        {
                ...
                else
                {
                    if (Application.isPlaying)
                    {
                        ...
                        if ( m_MovingPlatform.transform.parent != null )
                        {
                            Handles.DrawDottedLine( worldPos, m_MovingPlatform.transform.parent.transform.TransformPoint( m_MovingPlatform.worldNode[i - 1] ), 10 );
                        }
                        else
                        {
                            Handles.DrawDottedLine( worldPos, m_MovingPlatform.worldNode[i - 1], 10 );
