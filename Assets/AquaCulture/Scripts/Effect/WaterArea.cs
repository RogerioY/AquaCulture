...
namespace AquaCulture
{
    ...
    public class WaterArea : MonoBehaviour
    {
        ...
        public void FillLikeContainer( float MeasureParameter )
        {
            // We want to the water level to go up or down while fixing the bottom of the volume so that doesn't
            // appear to move in world space.
            float Difference = MeasureParameter - size.y;

            // Set this new size and offset.
            size.y = MeasureParameter;
            offset.y = 0.5f * MeasureParameter;

            // Shift the local position
            //transform.localPosition += new Vector3( 0.0f, 0.5f * Difference, 0.0f );

            AdjustComponentSizes();
            RecomputeMeshRetainShape();

            // I won't set this as dirty here as I think it might hammer the filesystem (though I don't think it will cost much)
        }

        // This method is *way* too slow, but it'll do for now. Optimisation candidate.
        public void RecomputeMeshRetainShape()
        {
            ...
            // Keep a reference to the previous columns for the scope of this function, hope they will be cleaned up afterwards
            WaterColumn[] PrevColumns = new WaterColumn[m_Columns.Length];
            m_Columns.CopyTo( PrevColumns, 0 );
            ...
            for ( int i = 0; i <= count; ++i )
            {
                ...
                // Odd points - these live vertically above Even points.
                ...
                m_Columns[i].velocity = PrevColumns[i].velocity;
                m_Columns[i].vertexIndex = i * 2 + 1;
                m_Columns[i].currentHeight = PrevColumns[i].currentHeight + ( m_Columns[i].baseHeight - PrevColumns[i].baseHeight );
                // Adjust the point relative to any height difference
                pts[m_Columns[i].vertexIndex].y = PrevColumns[i].currentHeight;
            }
    ...
    public class WaterAreaEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                m_WaterArea.RecomputeMeshRetainShape();
